...
- Camera extrinsics (pose per frame): R_cw, C_w, and 3x4 matrix E = [R_cw | -R_cw*C_w]
...
Usage examples:
  ...
  python scripts/hypersim_camera_depth.py convert-depth --scene ai_001_001 --cam cam_00 --frame 0000 --out z_cam_asset.npz
...
def load_extrinsics(scene_dir_path: str, cam_name: str, frame_iiii: str):
    ...
    # Map keyframe -> frame.IIII if available
    ...
    if os.path.exists(frame_map_path):
        with h5py.File(frame_map_path, "r") as f:
            frame_indices = np.array(f["dataset"])  # N
...
def cmd_get_extrinsics(args):
    sdir = scene_dir(args.scene)
    R_cw, C_w, E = load_extrinsics(sdir, args.cam, args.frame)
    result = {
        "scene": args.scene,
        "cam": args.cam,
        "frame": args.frame,
...
def cmd_get_intrinsics(args):
    M_proj, (W, H), M_cam_from_uv = load_intrinsics_projection(args.scene)
    result = {
        "scene": args.scene,
...
def build_parser():
    ...
    p_ext = sub.add_parser(
        "get-extrinsics", help="Dump extrinsics (R_cw, C_w, E) for a frame as JSON"
    )
    p_ext.add_argument("--scene", required=True, help="Scene name, e.g., ai_001_001")
    p_ext.add_argument("--cam", required=True, help="Camera name, e.g., cam_00")
    p_ext.add_argument("--frame", required=True, help="Frame index 'IIII', e.g., 0000")
    ...
    p_int.add_argument("--scene", required=True, help="Scene name, e.g., ai_001_001")
    ...
    p_conv.add_argument("--scene", required=True, help="Scene name, e.g., ai_001_001")
